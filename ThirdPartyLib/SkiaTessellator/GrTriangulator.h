...
#endif
#include <Windows.h>
#include "ISkiaPolygonAccessor.h"
...
#define TRIANGULATOR_WIREFRAME 0
	...
	public:
		...
		static int PathToTriangles(ISkiaPolygonAccessor* pAccessor, bool* isLinear) {
			if (!pAccessor || pAccessor->PaIsEmpty()) {
				return 0;
			}
			SkArenaAlloc alloc(kArenaDefaultChunkSize);
			GrTriangulator triangulator(pAccessor, &alloc);
	...
	protected:
		GrTriangulator(ISkiaPolygonAccessor* pAccessor, SkArenaAlloc* alloc)
			: fPath(pAccessor), fAlloc(alloc) {
		}
		...
		void emitMonotonePoly(const MonotonePoly*) const;
		void emitTriangle(
			Vertex* prev, Vertex* curr, Vertex* next, int winding) const;
		void emitPoly(const Poly*) const;
