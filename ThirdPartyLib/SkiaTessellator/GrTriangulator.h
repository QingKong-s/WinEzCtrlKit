...
#endif
#include <Windows.h>
#include "ISkiaTessellationHost.h"

namespace pk {
    using namespace SkiaTessellator;
    struct SkRect;
...
#define TRIANGULATOR_WIREFRAME 0
    ...
    public:
        ...
        constexpr static uint32_t kInvalidVertexIndex = 0xFFFF'FFFF;

        static bool PathToTriangles(ITessellationHost* pHost, bool* isLinear) {
            if (!pHost || pHost->PaIsEmpty()) {
                return false;
            }
            SkArenaAlloc alloc(kArenaDefaultChunkSize);
            GrTriangulator triangulator(pHost, &alloc);
            Poly* polys = triangulator.pathToPolys(isLinear);
            triangulator.polysToTriangles(polys);
            return true;
        }
    ...
    protected:
        GrTriangulator(ITessellationHost* pHost, SkArenaAlloc* alloc)
            : fPath(pHost), fAlloc(alloc) {
        }
        ...
        void pathToContours(VertexList* contours, bool* isLinear) const;
        ...
        void emitMonotonePoly(const MonotonePoly*, uint32_t& index) const;
        void emitTriangle(
            Vertex* prev, Vertex* curr, Vertex* next, int winding) const;
        void emitPoly(const Poly*, uint32_t& index) const;
        ...
        Poly* pathToPolys(bool* isLinear) const;
        // returns number of indices
        static int64_t CountPoints(Poly* polys,
            SkPathFillType overrideFillType, /*out*/ int64_t& vertCount_);
        void polysToTriangles(Poly*) const;

        // FIXME: fPath should be plumbed through function parameters instead.
        ITessellationHost* fPath;
