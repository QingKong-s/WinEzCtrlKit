...
namespace pk {
	...
	static void emit_triangle(
		...
		ISkiaPolygonAccessor* pAccessor) {
		...
		data[0] = v0->fPoint.fX;
		data[1] = v0->fPoint.fY;
		data[2] = GrNormalizeByteToFloat(v0->fAlpha);

		data[3] = v1->fPoint.fX;
		data[4] = v1->fPoint.fY;
		data[5] = GrNormalizeByteToFloat(v1->fAlpha);

		data[6] = v2->fPoint.fX;
		data[7] = v2->fPoint.fY;
		data[8] = GrNormalizeByteToFloat(v2->fAlpha);

		pAccessor->PaAppendTriangle(data);
		//emit_vertex(v0, emitCoverage, data);
		//emit_vertex(v1, emitCoverage, data);
		//emit_vertex(v2, emitCoverage, data);
#endif
	...
	void GrTriangulator::emitMonotonePoly(const MonotonePoly* monotonePoly) const {
	...
	void GrTriangulator::pathToContours(
		...
		*isLinear = true;
		...
		{
			switch (PathType((BYTE)types[i] & (BYTE)PathType::Mask))
			{
			case PathType::Move:
				if (contour->fHead) {
					contour++;
	...
	static SkRect getFlattenDataBounds(ISkiaPolygonAccessor* pAccessor) {
		SkRect bounds{};
		bounds.fLeft = bounds.fTop = FLT_MAX;
		const SkPoint* points;
		UINT count;
		pAccessor->PaGetFlattenData(&points, nullptr, &count);
		for (UINT i = 0; i < count; ++i) {
	...
	static int get_contour_count(ISkiaPolygonAccessor* pAccessor) {
		...
		const PathType* types;
		UINT count;
		pAccessor->PaGetFlattenData(&points, &types, &count);
		for (UINT i = 0; i < count; ++i)
		{
			switch (PathType((BYTE)types[i] & (BYTE)PathType::Mask))
			{
	...
	int GrTriangulator::polysToTriangles(Poly* polys) const {
		int64_t count64 = CountPoints(polys, fPath->PaGetFillType());
		if (0 == count64 || count64 > PK_MaxS32) {
			return 0;
		}
		int count = (int)count64;

		fPath->PaReserveVertex(count);

		TESS_LOG("emitting %d verts\n", count);
		polysToTriangles(polys, fPath->PaGetFillType());
		return static_cast<int>(fPath->PaGetVertexCount());
	}
}  // namespace pk
