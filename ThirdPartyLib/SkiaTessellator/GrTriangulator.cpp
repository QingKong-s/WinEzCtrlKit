...
namespace pk {
    ...
    static void emit_triangle(
        Vertex* v0, Vertex* v1, Vertex* v2, bool emitCoverage,
        ITessellationHost* pHost) {
        ...
        emit_vertex(v0, emitCoverage, data);
#else
        pHost->TslAppendTriangle(v0->fIndex, v1->fIndex, v2->fIndex);
        //emit_vertex(v0, emitCoverage, data);
        //emit_vertex(v1, emitCoverage, data);
        //emit_vertex(v2, emitCoverage, data);
#endif
    ...
    bool GrTriangulator::Edge::intersect(const Edge& other, SkPoint* p, uint8_t* alpha) const {
        ...
        if (alpha) {
            if (fType == EdgeType::kInner || other.fType == EdgeType::kInner) {
                ...
                *alpha = (uint8_t)std::max((1.0 - s) * fTop->fAlpha + s * fBottom->fAlpha,
    ...
    void GrTriangulator::emitMonotonePoly(const MonotonePoly* monotonePoly, uint32_t& index) const {
        ...
        uint32_t indexTemp = index;
        while (v != vertices.fTail) {
            Vertex* prev = v->fPrev;
            Vertex* curr = v;
            Vertex* next = v->fNext;

            if (prev->fIndex == kInvalidVertexIndex)
            {
                prev->fIndex = indexTemp++;
                fPath->TslAppendVertex(prev->fPoint.x(), prev->fPoint.y(),
                    GrNormalizeByteToFloat(prev->fAlpha));
            }
            if (curr->fIndex == kInvalidVertexIndex)
            {
                curr->fIndex = indexTemp++;
                fPath->TslAppendVertex(curr->fPoint.x(), curr->fPoint.y(),
                    GrNormalizeByteToFloat(curr->fAlpha));
            }
            if (next->fIndex == kInvalidVertexIndex)
            {
                next->fIndex = indexTemp++;
                fPath->TslAppendVertex(next->fPoint.x(), next->fPoint.y(),
                    GrNormalizeByteToFloat(next->fAlpha));
            }

            if (count == 3) {
                this->emitTriangle(prev, curr, next, monotonePoly->fWinding);
                index = indexTemp;
                return;
            }
    ...
    void GrTriangulator::emitTriangle(
        Vertex* prev, Vertex* curr, Vertex* next, int winding) const {
        if (winding > 0) {
        ...
        if (fCollectBreadcrumbTriangles && abs(winding) > 1 &&
            fPath->PaGetFillType() == SkPathFillType::kWinding) {
        ...
        emit_triangle(prev, curr, next, fEmitCoverage, fPath);
    }
    ...
    void GrTriangulator::emitPoly(const Poly* poly, uint32_t& index) const {
        if (poly->fCount < 3) {
            return;
        }
        TESS_LOG("emit() %d, size %d\n", poly->fID, poly->fCount);
        for (MonotonePoly* m = poly->fHead; m != nullptr; m = m->fNext) {
            this->emitMonotonePoly(m, index);
        }
    }
    ...
    void GrTriangulator::pathToContours(
        ...
        *isLinear = true;
        ...
        const PathType* types;
        UINT count;
        fPath->PaGetFlattenData(&points, &types, &count);
        bool fill = false;
        for (UINT i = 0; i < count; ++i)
        {
            if ((BYTE)types[i] & (BYTE)PathType::Begin)
            {
                fill = !!((BYTE)types[i] & (BYTE)PathType::Fill);
                if (contour->fHead) {
                    contour++;
                }
                this->appendPointToContour(points[i], contour);
                continue;
            }
            ...
            assert((BYTE)types[i] & (BYTE)PathType::Line);
            this->appendPointToContour(points[i], contour);
        }
    }
    ...
    bool GrTriangulator::applyFillType(int winding) const {
        return apply_fill_type(fPath->PaGetFillType(), winding);
    }
...
#pragma warning(suppress:28182)// Dereferencing NULL pointer
    ...
    static SkRect getFlattenDataBounds(ITessellationHost* pHost) {
        SkRect bounds{};
        bounds.fLeft = bounds.fTop = FLT_MAX;
        const SkPoint* points;
        UINT count;
        pHost->PaGetFlattenData(&points, nullptr, &count);
        for (UINT i = 0; i < count; ++i) {
            bounds.fLeft = std::min(bounds.fLeft, points[i].x());
            bounds.fTop = std::min(bounds.fTop, points[i].y());
            bounds.fRight = std::max(bounds.fRight, points[i].x());
            bounds.fBottom = std::max(bounds.fBottom, points[i].y());
        }
        return bounds;
    }
    ...
    Poly* GrTriangulator::contoursToPolys(VertexList* contours, int contourCnt) const {
        SkRect pathBounds;
        if (FAILED(fPath->PaGetBounds(&pathBounds)))
            pathBounds = getFlattenDataBounds(fPath);
    ...
    void GrTriangulator::polysToTriangles(Poly* polys,
        SkPathFillType overrideFillType, uint32_t& index) const {
        for (Poly* poly = polys; poly; poly = poly->fNext) {
            if (apply_fill_type(overrideFillType, poly)) {
                this->emitPoly(poly, index);
            }
        }
    }

    static int get_contour_count(ITessellationHost* pHost) {
        ...
        const PathType* types;
        UINT count;
        pHost->PaGetFlattenData(&points, &types, &count);
        for (UINT i = 0; i < count; ++i)
        {
            if ((BYTE)types[i] & ((BYTE)PathType::Begin | (BYTE)PathType::Fill))
            {
                if (!first) {
                    ++contourCnt;
                }
    ...
    Poly* GrTriangulator::pathToPolys(bool* isLinear) const {
        int contourCnt = get_contour_count(fPath);
        if (contourCnt <= 0) {
            *isLinear = true;
            return nullptr;
        }

        if (SkPathFillType_IsInverse(fPath->PaGetFillType())) {
            contourCnt++;
        }
        std::unique_ptr<VertexList[]> contours(new VertexList[contourCnt]);

        this->pathToContours(contours.get(), isLinear);
        return this->contoursToPolys(contours.get(), contourCnt);
    }

    int64_t GrTriangulator::CountPoints(Poly* polys,
        SkPathFillType overrideFillType,  /*out*/ int64_t& vertCount_) {
        int64_t idxcount = 0;
        int64_t vertCount = 0;
        for (Poly* poly = polys; poly; poly = poly->fNext) {
            if (apply_fill_type(overrideFillType, poly) && poly->fCount >= 3) {
                idxcount += (poly->fCount - 2) * (TRIANGULATOR_WIREFRAME ? 6 : 3);
                vertCount += poly->fCount;
            }
        }
    ...
    void GrTriangulator::polysToTriangles(Poly* polys) const {
        int64_t vertCount = 0;
        int64_t idxCount = CountPoints(polys, fPath->PaGetFillType(), vertCount);
        if (0 == idxCount || idxCount > PK_MaxS32) {
            return;
        }

        fPath->TslReserveVertex((UINT)vertCount);
        fPath->TslReserveIndex((UINT)idxCount);

        TESS_LOG("emitting %d verts\n", (int)idxCount);
        uint32_t index = fPath->TslGetVertexCount();
        polysToTriangles(polys, fPath->PaGetFillType(), index);
    }
}  // namespace pk
