...
namespace pk {
            ...
            for (Edge* e = v->fLastEdgeAbove; e && e != leftEnclosingEdge;) {
                ...
                if (leftOverlap && rightOverlap) {
                    ...
                    if (!isOuterBoundary) {
                        ...
                        e->fWinding = (int)PkScalarCopySign(
                            e->fType == EdgeType::kInner ? 2.f : 1.f, (float)e->fWinding);
                    }
                }
                e = prev;
            }
        ...
        for (Edge* e = boundary->fHead; e != nullptr; e = e->fRight) {
            ...
            if (!prevEdge->fLine.nearParallel(e->fLine) && prevInner.intersect(inner, &innerPoint) &&
                ...
                if (cosAngle < -kCosMiterAngle) {
                    ...
                    Line tangent(v->fPoint, v->fPoint + SkPoint::Make(
                        (SkScalar)bisector.fA, (SkScalar)bisector.fB));
                    ...
                    if (prevNormal.cross(normal) > 0) {
                        ...
                        Line prevTangent(prevV->fPoint,
                            prevV->fPoint + SkVector::Make(
                                (SkScalar)prevOuter.fA, (SkScalar)prevOuter.fB));
                        Line nextTangent(nextV->fPoint,
                            nextV->fPoint + SkVector::Make(
                                (SkScalar)outer.fA, (SkScalar)outer.fB));
                        ...
                        Line prevTangent(prevV->fPoint,
                            prevV->fPoint + SkVector::Make(
                                (SkScalar)prevInner.fA, (SkScalar)prevInner.fB));
                        Line nextTangent(nextV->fPoint,
                            nextV->fPoint + SkVector::Make(
                                (SkScalar)inner.fA, (SkScalar)inner.fB));
    ...
    void GrAATriangulator::polysToAATriangles(Poly* polys) const {
        int64_t vertCount;
        int64_t idxCount = CountPoints(polys, SkPathFillType::kWinding, vertCount);
        // Count the points from the outer mesh.
        for (Vertex* v = fOuterMesh.fHead; v; v = v->fNext) {
            for (Edge* e = v->fFirstEdgeBelow; e; e = e->fNextEdgeBelow) {
                idxCount += TRIANGULATOR_WIREFRAME ? 12 : 6;
                vertCount += TRIANGULATOR_WIREFRAME ? 4 : 2;
            }
        }
        if (0 == idxCount || idxCount > INT32_MAX) {
            return;
        }
        ...
        this->polysToTriangles(polys, SkPathFillType::kWinding, index);
        ...
        for (Vertex* v = fOuterMesh.fHead; v; v = v->fNext) {
            for (Edge* e = v->fFirstEdgeBelow; e; e = e->fNextEdgeBelow) {
                ...
                if (v0->fIndex == kInvalidVertexIndex)
                {
                    v0->fIndex = index++;
                    fPath->TslAppendVertex(v0->fPoint.x(), v0->fPoint.y(),
                        GrNormalizeByteToFloat(v0->fAlpha));
                }
                if (v1->fIndex == kInvalidVertexIndex)
                {
                    v1->fIndex = index++;
                    fPath->TslAppendVertex(v1->fPoint.x(), v1->fPoint.y(),
                        GrNormalizeByteToFloat(v1->fAlpha));
                }
                if (v2->fIndex == kInvalidVertexIndex)
                {
                    v2->fIndex = index++;
                    fPath->TslAppendVertex(v2->fPoint.x(), v2->fPoint.y(),
                        GrNormalizeByteToFloat(v2->fAlpha));
                }
                if (v3->fIndex == kInvalidVertexIndex)
                {
                    v3->fIndex = index++;
                    fPath->TslAppendVertex(v3->fPoint.x(), v3->fPoint.y(), v3->fAlpha);
                }
                this->emitTriangle(v0, v1, v2, 0 /*winding*/);
                this->emitTriangle(v0, v2, v3, 0 /*winding*/);
            }
        }
    }
}  // namespace pk
