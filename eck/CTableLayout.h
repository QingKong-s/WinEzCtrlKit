...
private:
    ...
    struct ITEM : ITEMBASE
    {
        ...
        constexpr ITEM(ILayout* pCtrl, const MARGINS& Margin, UINT uFlags, short cx, short cy,
            int idxRow, int idxCol, int nRowSpan, int nColSpan) noexcept
...
public:
    size_t Add(ILayout* pCtrl, int idxRow, int idxCol, const MARGINS& Mar = {},
        UINT uFlags = 0, int nRowSpan = 0, int nColSpan = 0) noexcept
    {
    ...
    void SetRowCol(int cRow, int cCol) noexcept
    {
        m_cRow = cRow;
        m_cCol = cCol;
    ...
    CELL& GetCell(int idxRow, int idxCol) noexcept
    {
    ...
    // 单元格的尺寸修改后，需要调用此函数刷新
    void CommitTableMetrics() noexcept
    {
        EckCounter(m_cRow, i)
        {
            m_Table[i][0].x = 0;
    ...
    EckInlineCe void SetCellMode(TlCellMode e) noexcept { m_eCellSizeMode = e; }
    EckInlineNdCe TlCellMode GetCellMode() const noexcept { return m_eCellSizeMode; }
    ...
    void SetColWidth(int idxCol, int cx) noexcept
    {
        EckAssert(idxCol < m_cCol);// 超出表格范围
        for (int i = 0; i < m_cRow; ++i)
            m_Table[i][idxCol].cx = cx;
    }
    void SetColWidth(int cx) noexcept
    {
        for (int i = 0; i < m_cCol; ++i)
            SetColWidth(i, cx);
    }
    void SetRowHeight(int idxRow, int cy) noexcept
    {
        EckAssert(idxRow < m_cRow);// 超出表格范围
        for (int i = 0; i < m_cCol; ++i)
            m_Table[idxRow][i].cy = cy;
    }
    void SetRowHeight(int cy) noexcept
    {
        for (int i = 0; i < m_cRow; ++i)
            SetRowHeight(i, cy);
    }
