...
EckInline HRESULT DrawListViewColumnDetail(HTHEME hTheme,
    HDC hDC, int x, int yTop, int yBottom) noexcept
{
    const RECT rc{ x - 2,yTop,x,yBottom };
    return DrawThemeBackground(hTheme, hDC, LVP_COLUMNDETAIL, 0, &rc, nullptr);
}

inline void DrawSelectionRect(HDC hDC, const RECT& rc) noexcept
{
...
inline void DrawSelectionRect(HDC hDC, const RECT& rc, int cxFrame) noexcept
{
    HDC hCDC = CreateCompatibleDC(hDC);
    HBITMAP hBitmap = CreateCompatibleBitmap(hDC, 1, 1);
    SelectObject(hCDC, hBitmap);
...
inline void DrawPlusMinusGlyph(HDC hDC, BOOL bPlus, const RECT& rc,
    COLORREF crBorder, COLORREF crSign) noexcept
{
