...
protected:
    ...
    static void CalcCtrlPosSize(const ITEMBASE& e, const RCWH& cell,
        _Out_ int& x, _Out_ int& y, _Out_ int& cx, _Out_ int& cy) noexcept
    {
        if (e.uFlags & LF_FIT)
        {
    ...
    static void MoveCtrlPosSize(const ITEMBASE& e, HDWP& hDwp,
        int x, int y, int cx, int cy) noexcept
    {
    ...
    void ReCalcDpiSize(ITEMBASE& e, int iDpiNew) noexcept
    {
...
public:
    EckInlineNdCe static UINT GetAlignFromFlags(UINT u) noexcept { return GetLowNBits(u, 4); }
    EckInlineNdCe static UINT GetSingleAlignFromFlags(UINT u) noexcept { return GetLowNBits(u, 2); }
    EckInlineNdCe static UINT GetCommFlagsFromFlags(UINT u) noexcept { return GetLowNBits(u, 9); }
    ...
    POINT LoGetPos() noexcept override { return { m_x,m_y }; }
    SIZE LoGetSize() noexcept override { return { m_cx,m_cy }; }
    void LoShow(BOOL bShow) noexcept override {}
    SIZE LoGetAppropriateSize() noexcept override { return { m_cx,m_cy }; }
    ...
    EckInline void Arrange(int cx, int cy) noexcept
    {
        CLayoutBase::LoSetPosSize(0, 0, cx, cy);
        LoCommit();
    }

    EckInline void Arrange(int x, int y, int cx, int cy) noexcept
    {
    ...
    HDWP PreArrange(size_t cWindows = 2u) noexcept
    {
#ifdef _DEBUG
    ...
    void PostArrange(HDWP hDwp) noexcept
    {
#ifdef _DEBUG
        EckAssert(m_bPrePostBalanced);
