    ...
    int iTzHour = 0, int iTzMinute = 0) noexcept
{
    ...
    if (st.wMonth)
        rs.PushBackFormat("-%02d", st.wMonth);
    if (st.wDay)
        rs.PushBackFormat("-%02d", st.wDay);
    if (st.wHour || st.wMinute || st.wSecond || st.wMilliseconds)
    {
        rs.PushBackFormat("T%02d", st.wHour);
        if (st.wMinute)
            rs.PushBackFormat(":%02d", st.wMinute);
        if (st.wSecond)
            rs.PushBackFormat(":%02d", st.wSecond);
        if (st.wMilliseconds)
            rs.PushBackFormat(".%03d", st.wMilliseconds);
    }
    if (iTzHour == INT_MIN)
        rs.PushBackChar('Z');
    else if (iTzHour)
    {
        if (iTzHour > 0)
            rs.PushBackFormat("+%02d", iTzHour);
        else
            rs.PushBackFormat("-%02d", -iTzHour);
        if (iTzMinute)
            rs.PushBackFormat(":%02d", iTzMinute);
    }
}
ECK_NAMESPACE_END