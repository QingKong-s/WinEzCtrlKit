...
inline NTSTATUS UiaTryAcquire(_Out_ UIA& uia)
{
	NTSTATUS nts;
	DWORD dwSid;
	ULONG cbRet;
	...
	nts = EnumProcess([&](SYSTEM_PROCESS_INFORMATION* pspi)
		{
...
EckInline void UiaRelease(const UIA& uia)
{
	if (uia.hTokenWinlogon)
		NtClose(uia.hTokenWinlogon);
}

inline NTSTATUS UiaRestart(const UIA& uia)
{
	NTSTATUS nts;
