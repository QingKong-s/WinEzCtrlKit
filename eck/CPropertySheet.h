...
public:
    ...
    {
        return (BOOL)SendMessageW(PSM_ADDPAGE, 0, (LPARAM)hPage);
    }
    ...
    {
        return (BOOL)SendMessageW(PSM_APPLY, 0, 0);
    }
    ...
    {
        SendMessageW(PSM_CANCELTOCLOSE, 0, 0);
    }

    EckInline void Changed(HPROPSHEETPAGE hPage) const noexcept
    {
        SendMessageW(PSM_CHANGED, (WPARAM)hPage, 0);
    }
    ...
    {
        SendMessageW(PSM_ENABLEWIZBUTTONS, uButtons, uMask);
    }
    ...
    {
        SendMessageW(PSM_ENABLEWIZBUTTONS, bEnable ? uButtons : 0, uButtons);
    }

    EckInline HWND GetCurrentPageWindow() const noexcept
    {
        return (HWND)SendMessageW(PSM_GETCURRENTPAGEHWND, 0, 0);
    }

    EckInline int GetResult() const noexcept
    {
        return (int)SendMessageW(PSM_GETRESULT, 0, 0);
    }

    EckInline HWND GetTabControl() const noexcept
    {
        return (HWND)SendMessageW(PSM_GETTABCONTROL, 0, 0);
    }

    EckInline int HWndToIndex(HWND hDlg) const noexcept
    {
        return (int)SendMessageW(PSM_HWNDTOINDEX, (WPARAM)hDlg, 0);
    }

    EckInline int IdToIndex(PCWSTR idRes) const noexcept
    {
        return (int)SendMessageW(PSM_IDTOINDEX, 0, (LPARAM)idRes);
    }

    EckInline HWND IndexToHWnd(int idx) const noexcept
    {
        return (HWND)SendMessageW(PSM_INDEXTOHWND, idx, 0);
    }

    EckInline PCWSTR IndexToId(int idx) const noexcept
    {
        return (PCWSTR)SendMessageW(PSM_INDEXTOID, idx, 0);
    }

    EckInline HPROPSHEETPAGE IndexToHPage(int idx) const noexcept
    {
        return (HPROPSHEETPAGE)SendMessageW(PSM_INDEXTOPAGE, idx, 0);
    }

    EckInline BOOL InsertPage(HPROPSHEETPAGE hPage,
        HPROPSHEETPAGE hInsertAfter = nullptr) const noexcept
    {
        return (BOOL)SendMessageW(PSM_INSERTPAGE, (WPARAM)hInsertAfter, (LPARAM)hPage);
    }

    EckInline BOOL InsertPage(HPROPSHEETPAGE hPage, int idx) const noexcept
    {
        return (BOOL)SendMessageW(PSM_INSERTPAGE, idx, (LPARAM)hPage);
    }

    EckInline BOOL IsPsDialogMessage(MSG* pmsg) const noexcept
    {
        return (BOOL)SendMessageW(PSM_ISDIALOGMESSAGE, 0, (LPARAM)pmsg);
    }

    EckInline int HPageToIndex(HPROPSHEETPAGE hPage) const noexcept
    {
        return (int)SendMessageW(PSM_PAGETOINDEX, 0, (LPARAM)hPage);
    }

    EckInline void PressButton(UINT uButton) const noexcept
    {
        SendMessageW(PSM_PRESSBUTTON, uButton, 0);
    }
    ...
    {
        SendMessageW(PSM_SETTITLEW, bPropOf ? PSH_PROPTITLE : 0, (LPARAM)pszText);
    }
    ...
    {
        SendMessageW(PSM_SETWIZBUTTONS, bShieldIcon ? PSWIZBF_ELEVATIONREQUIRED : 0, uButtons);
    }
    ...
    {
        SendMessageW(PSM_SHOWWIZBUTTONS, uButtons, uMask);
    }
    ...
    {
        SendMessageW(PSM_SHOWWIZBUTTONS, bShow ? uButtons : 0, uButtons);
    }

    EckInline void Unchanged(HPROPSHEETPAGE hPage) const noexcept
    {
        SendMessageW(PSM_UNCHANGED, (WPARAM)hPage, 0);
    }
};
ECK_NAMESPACE_END