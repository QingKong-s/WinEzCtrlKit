    ...
    _Out_ TInt& i, int iRadix = 0, _Outptr_opt_ TPtr* EndPtr = nullptr)
{
    ...
    if (iRadix == 0)
        if (Str + 2 < pEnd &&
            (*Str == '0' && (Str[1] == 'x' || Str[1] == 'X')))// 0x, 0X
        {
            Str += 2;
            iRadix = 16;
        }
        else if (Str + 2 < pEnd &&
            *Str == '0' && (Str[1] == 'b' || Str[1] == 'B'))// 0b, 0B
        {
            Str += 2;
            iRadix = 2;
        }
...
constexpr inline size_t TableI32StringSize[]{ 0,32,32,21,16,14,13,12,11,11,10,10,9,9,9,9,8,8,8,8,8,8,8,8,7,7,7,7,7,7,7,7,7,7,7,7 };
constexpr inline size_t TableI64StringSize[]{ 0,64,64,41,32,28,25,23,22,21,20,19,18,18,17,17,16,16,16,16,15,15,15,15,14,14,14,14,14,14,14,13,13,13,13,13 };

template<class TInt>
EckInlineNdCe size_t TcsCvtCalcBufferSize(int iRadix = 10, int cchFillTo = 0)
{
    if constexpr (sizeof(TInt) == 8)
        return std::max(TableI64StringSize[iRadix], (size_t)cchFillTo) + 2;
    else
...
inline TcsCvtErr TcsFromInt(_Out_writes_(Len) TPtr Str,
    ...
    TPtr* ppEnd = nullptr,
    int cchFillTo = 0, RemoveStdCharPtr_T<TPtr> chFill = '0')
{
    EckAssert(iRadix >= 2 && iRadix <= 36);
    EckAssert(Len >= TcsCvtCalcBufferSize<TInt>(iRadix, cchFillTo) - 1);
    const auto pEnd = Str + Len;
    using TUnsigned = std::make_unsigned_t<TInt>;
    ...
    if (bNeg)
        *Str++ = '-';
    const auto pFillEnd = Str + cchExtraFill;
    if (pFillEnd > pWrite)
    {
        if (ppEnd) *ppEnd = nullptr;
        return TcsCvtErr::BufferTooSmall;
    }
    for (; Str < pFillEnd; ++Str)
        *Str = chFill;
    if (Str != pWrite)
        TcsMoveLen(Str, pWrite, cchNum);
    Str += cchNum;
    if (Str < pEnd) *Str = 0;
    if (ppEnd) *ppEnd = Str;
    return TcsCvtErr::Ok;
