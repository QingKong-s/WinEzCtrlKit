...
private:
    ...
    void QueryAtlasInfomation(int iPart, DWMW_PART_INFO& Info) noexcept
    {
...
public:
    ...
    void AnalyzeTheme(HTHEME hTheme) noexcept
    {
        Clear();
        m_hTheme = hTheme;
        for (int i = 0; i < PartCount; ++i)
            QueryAtlasInfomation(i, m_Item[i]);
    }

    HRESULT AnalyzeDefaultTheme() noexcept
    {
        Clear();
        PWSTR pszWinDir;
        HRESULT hr;
...
#pragma warning(suppress:6255)
    ...
    BOOL GetPartRect(RECT& rc, RECT& rcBkg, DwmWndPart ePart, DwmWPartState eState,
        BOOL bDark, BOOL bActive, int iDpi, DWMW_GET_PART_EXTRA* pExtra = nullptr) const noexcept
    {
    ...
    EckInline constexpr const auto& AtPart(int idx) const noexcept { return m_Item[idx]; }

    EckInline constexpr HTHEME GetHTheme() const noexcept { return m_hTheme; }
};
ECK_NAMESPACE_END