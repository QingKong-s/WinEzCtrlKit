...
inline HRESULT BecCreateNoiseBrush(const becwuc::Compositor& Compositor,
    becwuc::CompositionSurfaceBrush& NoiseBrush) noexcept
{
...
namespace Priv
{
    ...
    inline BecHsv BecpRgbToHsv(const BecRgb& rgb) noexcept
    {
        double hue = 0;
        double saturation = 0;
        double value = 0;
    ...
    inline BecRgb BecpHsvToRgb(const BecHsv& hsv) noexcept
    {
        double hue = hsv.h;
    ...
    inline double BecpGetTintOpacityModifier(const D2D1_COLOR_F& tintColor) noexcept
    {
    ...
    inline void BecpGetLuminosityColor(_Inout_ D2D1_COLOR_F& tintColor,
        std::optional<double> luminosityOpacity) noexcept
    {
...
EckInline void BecGetEffectiveTintColor(_Inout_ D2D1_COLOR_F& TintColor,
    float TintOpacity, bool TintLuminosityOpacity) noexcept
{
...
EckInline void BecGetEffectiveLuminosityColor(_Inout_ D2D1_COLOR_F& TintColor,
    float TintOpacity, std::optional<float> TintLuminosityOpacity) noexcept
{
    TintColor.a *= TintOpacity;
...
struct CBecAcrylic
{
    ...
    */
    ...
    winrt::com_ptr<CWinRtD2DInteropFx> GetOutputEffect() const noexcept { return FxBlendNoise; }

    void Connect(
        const D2D1_COLOR_F& TintColor,
        const D2D1_COLOR_F& LuminosityColor,
        float fStandardDeviation = 20.f,
        float fNoiseOpacity = 0.02f) noexcept
    {
    ...
    becwuc::CompositionEffectBrush CreateBrush(
        ...
        becwuc::CompositionEffectFactory* pEffectFactory = nullptr) const noexcept
    {
...
struct CBecAcrylicLegacy
{
    ...
    */
    ...
    void Connect(
        ...
        float fSaturation = 1.25f) noexcept
    {
        FxBorder->SetInput(0, NNoise);
        FxBorder->SetValueT(D2D1_BORDER_PROP_EDGE_MODE_X, D2D1_BORDER_EDGE_MODE_WRAP);
        FxBorder->SetValueT(D2D1_BORDER_PROP_EDGE_MODE_Y, D2D1_BORDER_EDGE_MODE_WRAP);
...
struct CBecAero
{
    ...
    */
    ...
    void Connect(
        ...
        float fBlurOpacity = 0.49f) noexcept
    {
...
struct CBecBlur
{
    ...
    */
    ...
    void Connect(const D2D1_COLOR_F& TintColor, float fStandardDeviation = 6.f) noexcept
    {
        FxBlur->SetInput(0, NBackdrop);
        FxBlur->SetValueT(D2D1_GAUSSIANBLUR_PROP_STANDARD_DEVIATION, fStandardDeviation);
        FxBlur->SetValueT(D2D1_GAUSSIANBLUR_PROP_BORDER_MODE, D2D1_BORDER_MODE_HARD);
