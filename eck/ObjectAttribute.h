...
#define ECK_OBJA_INT(Name, Field) \
	if (::eck::TcsCompareLength2I(svName.data(), svName.size(), EckArgString(Name)) == 0) { \
		if (bSet) { \
			int v; \
...
inline ObjAttrErr OamFromString(std::wstring_view Str, TOamValFrom<E> Val) noexcept
{
    ...
    {
        if (TcsCompareLength2I(Str.data(), Str.size(), EckArgString(L"true")) == 0 ||
            TcsCompareLength2(Str.data(), Str.size(), EckArgString(L"1")) == 0 ||
            TcsCompareLength2(Str.data(), Str.size(), EckArgString(L"真")) == 0)
            Val = TRUE;
        else
            Val = FALSE;
        return ObjAttrErr::Ok;
