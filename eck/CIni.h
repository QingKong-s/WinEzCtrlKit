...
private:
		...
		int* pposSectionEnd = nullptr, int* pposKeyBegin = nullptr) const
	{
		...
		if (m_bParseEscapeChar)
		{
			rsUnescaping = pszSection;
			ProcessUnEscapeChar(rsUnescaping);
	...
	int FindValueEndPos(int posValueBegin)
	{
	...
	BOOL WriteValue(PCWSTR pszSection, PCWSTR pszKey, PCWSTR pszValue,
		BOOL bMustExist = FALSE, BOOL bUnEscapeChar = TRUE)
	{
		int pos1, posSectionEnd;
		CRefStrW rsNew{}, rsUnescaping{};
		int pos0 = FindValuePos(pszSection, pszKey, &posSectionEnd);
		if (pos0 == StrNPos)
		{
			if (bMustExist)
				return FALSE;
			else if (posSectionEnd == StrNPos)// 没有节
			{
				...
				if (m_bParseEscapeChar && bUnEscapeChar)
				{
					rsUnescaping = pszSection;
					ProcessUnEscapeChar(rsUnescaping);
					pszSection = rsUnescaping.Data();
		...
		pos1 = FindValueEndPos(pos0);
		m_rsText.Replace(pos0, pos1 - pos0, pszValue, (int)wcslen(pszValue));
		return TRUE;
	}
public:
	static void ProcessEscapeChar(CRefStrW& rsText, int posBegin = 0)
	{
	...
	static void ProcessUnEscapeChar(CRefStrW& rsText, int posBegin = 0)
	{
	...
	CIni() = default;
	CIni(PCWSTR pszFileName, UINT uCp = CP_UTF8)
	{
		Load(pszFileName, uCp);
	}

	EckInline int ReadInt(PCWSTR pszSection, PCWSTR pszKey, int Default = 0) const
	{
		const int pos = FindValuePos(pszSection, pszKey);
		if (pos == StrNPos)
			return Default;
		return _wtoi(m_rsText.Data() + pos);
	}

	EckInline double ReadDouble(PCWSTR pszSection, PCWSTR pszKey, double Default = 0.0) const
	{
		const int pos = FindValuePos(pszSection, pszKey);
		if (pos == StrNPos)
			return Default;
	...
	BOOL ReadBool(PCWSTR pszSection, PCWSTR pszKey, BOOL Default = FALSE)
	{
		const int pos0 = FindValuePos(pszSection, pszKey);
		if (pos0 == StrNPos)
			return Default;
	...
	EckInline CRefStrW ReadString(PCWSTR pszSection, PCWSTR pszKey, PCWSTR Default = L"")
	{
		int pos0 = FindValuePos(pszSection, pszKey);
		if (pos0 == StrNPos)
			return Default;
		int pos1 = FindValueEndPos(pos0);
		auto rs = CRefStrW(m_rsText.Data() + pos0, pos1 - pos0);
		if (m_bParseEscapeChar)
			ProcessEscapeChar(rs);
		return rs;
	}

	BOOL ReadIntArray(std::vector<int>& v, PCWSTR pszSection, PCWSTR pszKey)
	{
		const int pos0 = FindValuePos(pszSection, pszKey);
		if (pos0 == StrNPos)
			return FALSE;
	...
	BOOL ReadDoubleArray(std::vector<double>& v, PCWSTR pszSection, PCWSTR pszKey)
	{
		const int pos0 = FindValuePos(pszSection, pszKey);
		if (pos0 == StrNPos)
			return FALSE;
	...
	BOOL ReadBoolArray(std::vector<TBool>& v, PCWSTR pszSection, PCWSTR pszKey)
	{
		const int pos0 = FindValuePos(pszSection, pszKey);
		if (pos0 == StrNPos)
			return FALSE;
	...
	BOOL ReadStringArray(std::vector<CRefStrW>& v, PCWSTR pszSection, PCWSTR pszKey)
	{
		int pos0 = FindValuePos(pszSection, pszKey);
		if (pos0 == StrNPos)
			return FALSE;
		...
		if (m_bParseEscapeChar)
			SplitStr(m_rsText.Data() + pos0 + 1, LR"(",")", 0, cch, 1,
				[&v](PCWSTR pszStart, int cchSub)
				{
					ProcessEscapeChar(v.emplace_back(pszStart, cchSub));
					return FALSE;
				});
		else
			SplitStr(m_rsText.Data() + pos0 + 1, LR"(",")", 0, cch, 1,
	...
	EckInline BOOL WriteDouble(PCWSTR pszSection, PCWSTR pszKey, double Value, BOOL bMustExist = FALSE)
	{
		auto rsValue = ToStr(Value, 15);
		return WriteValue(pszSection, pszKey, rsValue.Data(), bMustExist, FALSE);
	}

	EckInline BOOL WriteBool(PCWSTR pszSection, PCWSTR pszKey, BOOL Value, BOOL bMustExist = FALSE)
	{
		return WriteValue(pszSection, pszKey, Value ? L"True" : L"False", bMustExist, FALSE);
	}

	EckInline BOOL WriteString(PCWSTR pszSection, PCWSTR pszKey, PCWSTR pszValue, BOOL bMustExist = FALSE)
	{
		return WriteValue(pszSection, pszKey, pszValue, bMustExist);
	}

	BOOL WriteIntArray(PCWSTR pszSection, PCWSTR pszKey, const int* Value, int cValue, BOOL bMustExist = FALSE)
	{
		CRefStrW rs{};
		EckCounter(cValue, i)
			rs.AppendFormat(L"%d,", Value[i]);
		if (cValue)
			rs.PopBack();
		return WriteValue(pszSection, pszKey, rs.Data(), bMustExist, FALSE);
	}

	BOOL WriteDoubleArray(PCWSTR pszSection, PCWSTR pszKey, const double* Value, int cValue, BOOL bMustExist = FALSE)
	{
		CRefStrW rs{};
		EckCounter(cValue, i)
			rs.AppendFormat(L"%f,", Value[i]);
		if (cValue)
			rs.PopBack();
		return WriteValue(pszSection, pszKey, rs.Data(), bMustExist, FALSE);
	}
	...
	template<class TBool = BOOL>
	BOOL WriteIntArray(PCWSTR pszSection, PCWSTR pszKey, const TBool* Value, int cValue, BOOL bMustExist = FALSE)
	{
		CRefStrW rs{};
		rs.Reserve(6 * cValue);
		EckCounter(cValue, i)
	...
	BOOL WriteStringArray(PCWSTR pszSection, PCWSTR pszKey, const CRefStrW* Value, int cValue, BOOL bMustExist = FALSE)
	{
		CRefStrW rs{};
		if (m_bParseEscapeChar)
		{
		...
		else
		{
			EckCounter(cValue, i)
			{
				...
				rs.PushBack(Value[i]);
				if (!Value[i].IsEmpty())
					ProcessUnEscapeChar(rs, posBegin);
				rs.PushBack(L",");
			}
		}
		if (cValue)
	...
	BOOL DeleteSection(PCWSTR pszSection)
	{
		int pos0;
		CRefStrW rsUnescaping{};
		if (m_bParseEscapeChar)
	...
	BOOL Load(PCWSTR pszFileName, UINT uCp = CP_UTF8)
	{
		...
		if (uCp == CP_UTF16LE)
			m_rsText = CRefStrW((PCWSTR)rbFile.Data(), (int)(rbFile.Size() / sizeof(WCHAR)));
		else if (uCp == CP_UTF16BE)
			m_rsText = Utf16RevByte((PCWSTR)rbFile.Data(), (int)(rbFile.Size() / sizeof(WCHAR)));
		else
			m_rsText = StrX2W((PCSTR)rbFile.Data(), (int)rbFile.Size(), uCp);
		return TRUE;
	}

	BOOL Save(PCWSTR pszFileName, UINT uCp = CP_UTF8, BOOL bAddBom = TRUE)
	{
		CRefBin rb{};
		if (uCp == CP_UTF16LE)
		{
	...
	void SetParseEscapeChar(BOOL bParseEscapeChar) { m_bParseEscapeChar = bParseEscapeChar; }

	void SetParseComment(BOOL bParseComment) { m_bParseComment = bParseComment; }

	BOOL SetLineBreak(PCWSTR pszLineBreak)
	{
	...
	EckInline void SetText(PCWSTR pszText) { m_rsText = pszText; }

	EckInline void SetText(const CRefStrW& rsText) { m_rsText = rsText; }

	EckInline void SetText(CRefStrW&& rsText) { m_rsText = std::move(rsText); }

	EckInline void Clear() { m_rsText.Clear(); }
};
ECK_NAMESPACE_END